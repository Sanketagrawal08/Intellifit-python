...
#         # NEW: live session data buffer + start time
...
#         with col1:
#             patient_id = st.text_input("🆔 Patient ID*", placeholder="Enter patient id (eg 1,2..)")
...
#                             # ====== SAVE DATA IN INTERVALS DURING SESSION ======
#                             # Har 9th processed frame pe ek sample record
#                             if frame_count % 9 == 0:
#                                 st.session_state.live_analysis_buffer.append({
#                                     "timestamp": datetime.datetime.now().isoformat(),
#                                     "pelvic_tilt_angle": float(analysis_result.get("pelvic_tilt_angle", 0)),
#                                     "lateral_tilt_angle": float(analysis_result.get("lateral_tilt_angle", 0)),
#                                     "posture_score": float(analysis_result.get("posture_score", 0)),
#                                     "quality_score": float(analysis_result.get("quality_score", 0)),
#                                     "analysis_quality": analysis_result.get("analysis_quality", "unknown"),
#                                     "measurement_confidence": float(analysis_result.get("measurement_confidence", 0)),
#                                     "tm_label": analysis_result.get("tm_label", None),
#                                     "tm_confidence": float(analysis_result.get("tm_confidence", 0.0)),
#                                 })
#                             # ====================================================
...
#                                     st.metric(
#                                         "📊 MP Quality",
#                                         analysis_result.get('analysis_quality', 'Unknown').title()
#                                     )
...
#     # Only show summary when we have recorded data
#     if st.session_state.live_analysis_buffer:
#         st.markdown("### 📈 Live Session Summary")

#         pelvic_vals = [d["pelvic_tilt_angle"] for d in st.session_state.live_analysis_buffer]
#         lat_vals = [d["lateral_tilt_angle"] for d in st.session_state.live_analysis_buffer]
...
#         # Duration info (if start time recorded)
#         if st.session_state.live_session_start:
#             duration_sec = (datetime.datetime.now() - st.session_state.live_session_start).total_seconds()
#             st.caption(
#                 f"Session duration ~ {int(duration_sec)} seconds • "
#                 f"{len(st.session_state.live_analysis_buffer)} sampled measurements"
#             )

#         # Save entire session as one clinical entry
...
#                 "session_start": st.session_state.live_session_start.isoformat() if st.session_state.live_session_start else None,
#                 "session_end": datetime.datetime.now().isoformat(),
#             }

#             entry = st.session_state.patient.add_pelvic_analysis(
...
#             if save_patient_data(st.session_state.patient):
#                 st.success("✅ Live session saved to patient medical record (with full time-series data)!")
...
#     else:
#         st.info("No live session data recorded yet. Start an assessment to begin recording.")
...
#     # Build JSON structure exactly like the file
...
#     # ---------- SELECT WHICH ASSESSMENT FOR CLINICAL REPORT ----------
#     selected_entry = None
#     if pelvic_history:
...
#         # Generate the clinical text report using the backend generator
#         # (we use the "private" helper to avoid writing a file)
...
#     # ---------- TABS: JSON VIEW + CLINICAL REPORT ----------
#     tab_json, tab_report = st.tabs(["🧬 JSON Clinical Record", "📄 Clinical Text Report"])

#     # --- TAB 1: JSON ---
#     with tab_json:
#         st.markdown("#### 🧬 Full JSON Clinical Record")
#         st.json(patient_json)

#         json_bytes = json.dumps(patient_json, indent=2).encode("utf-8")
#         st.download_button(
#             label="📥 Download JSON Clinical Record",
#             data=json_bytes,
#             file_name=f"patient_{patient_json['patient_id']}_clinical_record.json",
#             mime="application/json",
#             use_container_width=True,
#             help="This is the same JSON that is saved in the patient_data folder."
#         )

#     # --- TAB 2: Clinical Text Report ---
#     with tab_report:
#         st.markdown("#### 📄 Formatted Clinical Report (Text)")

#         if not pelvic_history:
#             st.info("No pelvic assessments available yet. Perform an analysis to generate a clinical report.")
#         else:
#             # Read-only text area for the full report
#             st.text_area(
#                 "Clinical Report (read-only)",
...
#                 mime="text/plain",
#                 use_container_width=True,
#                 help="This is the same structured text report your backend generates."
#             )
...
def render_professional_sidebar():
    ...
    with st.sidebar:
        ...
        if st.button(
            "👤 Patient Management", use_container_width=True, key="nav_patients"
        ):
            st.session_state.selected_page = "patients"
            st.rerun()

        if st.button(
            "📐 Pelvic Analysis", use_container_width=True, key="nav_analysis"
        ):
            st.session_state.selected_page = "analysis"
            st.rerun()
...
def render_static_image_analysis():
    ...
    with col2:
        if uploaded_file and st.button(
            "🔍 Perform Clinical Analysis", type="primary", use_container_width=True
        ):
            with st.spinner("🔄 Processing advanced clinical analysis..."):
                # Convert and process image
                image_array = np.array(image)
