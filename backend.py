...
class IntelliFitPatient:
    ...
    def add_session(self, exercise_type, max_angle, reps_completed, pain_level, notes=""):
        """Add exercise session to patient record"""
        session = {
            'date': datetime.datetime.now(),
            'exercise_type': exercise_type,
            'max_rom_achieved': max_angle,
            'reps_completed': reps_completed,
            'pain_level': pain_level,
            'notes': notes,
            'session_duration': 0,  # Can be updated
            'therapist_notes': ""
        }
